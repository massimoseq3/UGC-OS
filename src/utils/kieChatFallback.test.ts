...
import { CHAT_MODEL_DEFAULT, getChatTarget } from './models'
...
describe('the chat fallback', () => {
  it('points the default chat model at Gemini 3.5 Flash', () => {
    const target = getChatTarget(CHAT_MODEL_DEFAULT)
  ...
  it('answers from the primary when it works, without touching the fallback', async () => {
    ...
    await expect(kieChatCompletions('k', getChatTarget(), messages)).resolves.toBe('from 3.8')
  ...
  it('retries once on the fallback when the primary fails', async () => {
    ...
    await expect(kieChatCompletions('k', getChatTarget(), messages)).resolves.toBe('from 3.5')
  ...
  it("surfaces the primary's error when both fail", async () => {
    ...
    await expect(kieChatCompletions('k', getChatTarget(), messages)).rejects.toThrow('upstream said 500')
  })

  it('does not fall back on a key or balance problem', async () => {
    const hits = stubKie({ [PRIMARY]: () => envelope(402), [FALLBACK]: () => ok('from 3.5') })
    await expect(kieChatCompletions('k', getChatTarget(), messages)).rejects.toThrow('upstream said 402')
    expect(hits).toEqual([PRIMARY])
  })

  it('does not fall back on a truncated answer', async () => {
    const hits = stubKie({ [PRIMARY]: () => ok('half a', 'length'), [FALLBACK]: () => ok('from 3.5') })
    await expect(kieChatCompletions('k', getChatTarget(), messages)).rejects.toBeInstanceOf(TruncatedResponseError)
    expect(hits).toEqual([PRIMARY])
  })
