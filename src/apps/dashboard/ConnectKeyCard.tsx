...
// from its foot. The BANNER vanishes the moment the key lands, but the guide
// and Settings must not go with it: the guide saves the key itself, so gating
// the whole card on "no key yet" unmounted it before its "You're Connected"
// state could show, and shut a Settings opened from it on Save. So the card
// gates only its own banner — render it unconditionally.

export default function ConnectKeyCard() {
  const needsKey = useSettingsStore((s) => s.kieApiKey.trim().length === 0)
