...
const WRITE_STYLE_INSTRUCTION: Record<WriteStyle, string> = {
  ...
  pov: 'STRUCTURE — POV: frame the whole clip as a "POV: ___" scenario the viewer is living right now. Open by speaking or acting the POV line in second person, present tense, so they\'re dropped INTO the moment instead of watching someone describe it. Pick the exact situation where the pain bites or the product saves the day, and stay inside the scene — react, talk to the camera like it\'s the other person in that moment. The product shows up as the thing that makes the moment land, not as a pitch. End on the feeling; a hard call-to-action is optional and often better dropped.',
  'day-in-life': 'STRUCTURE — DAY IN MY LIFE: narrate a slice of a real day like a casual vlog, moving through moments with rough times the way people actually talk ("okay it\'s like 7am", "that 3pm slump hits"). The product is NOT the subject — it\'s one honest beat inside the day, used the way you\'d really use it and mentioned once in passing, like you\'re barely selling. The realness of the routine is what sells; let the product earn its little moment instead of announcing it. Trail off into the next part of the day or land softly on the payoff.',
  grwm: 'STRUCTURE — GRWM (GET READY WITH ME): you\'re talking to the camera while physically getting ready (makeup, skincare, hair, outfit, heading somewhere). The hook is the REASON you\'re getting ready ("getting ready for the worst date of my life", "first day at the new job and I\'m terrified"). Let a story or a tangent run underneath the routine — the getting-ready is the b-roll, the talking is the content. The product enters at the exact step where you\'d genuinely reach for it, narrated as a normal part of the routine ("okay this is the part everyone keeps asking about"). Keep the story and the routine moving together and pay off the opening reason near the end.',
  tutorial: 'STRUCTURE — HOW-TO / TUTORIAL: promise a specific result in the hook ("how I got X in two weeks"), then actually teach it in a few quick steps. Talk like you\'re showing one friend, never lecturing. The product is the step that makes it work — don\'t hide it, but frame it as the tool that does the job, not the pitch. Keep the steps concrete and skimmable with no filler between them. End on the result they\'ll get and the product as the shortcut to it.',
  'whats-in-bag': 'STRUCTURE — WHAT\'S IN MY BAG / FAVES: a fast, casual roundup of the few things you actually carry or can\'t stop using right now. Run through the others quickly and a little offhand, then slow down and put your real excitement on the product as the standout. Make the filler items feel real and specific (the cheap pen, the half-dead lip balm) so the hero doesn\'t read as a planted ad. Save the product for the spot that gets the most genuine love, then a light call-to-action.',
  'tiktok-made-me': 'STRUCTURE — VIRAL / "MADE ME BUY IT": lead with caving to the hype ("okay I finally caved", "this was all over my feed so I bought it"). You were half-expecting to be let down. Then give the honest verdict: did it actually live up to it? Pay it off with one specific reason it did — a real detail, number, or moment — not vague praise. The social proof IS the hook: you\'re the latest person to try it and report back, with genuine, slightly-surprised energy. Quick call-to-action.',
}
