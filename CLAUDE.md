...
| System | Dashboard | `dashboard/` | Default landing page. Greeting (profile first name, Instrument Serif italic like all hero stats) + bento value metrics: time saved (per-kind minutes + a task-switching constant, both in `utils/usage.ts`), money saved vs the higher of official-API and creator-platform pricing, current/longest streak, active days, 26-week activity heatmap (green ramp), Get Credits/Community quick links, and crab-crew shortcut tiles (dock order, `ArrowUpRight` → `openApp`). Everything derives from the `usageDays` ledger (see Banks) — the app never writes data. The menu bar shows a flame "N day streak" chip (opens Dashboard) whenever the streak is live. |
