...
| Tools | Ad Analyzer | `ad-anatomy/` | Ad video (a file, or a pasted TikTok / Instagram / Meta link) → scorecard, breakdown, transcript, Scenes. → [`src/apps/ad-anatomy/CLAUDE.md`](src/apps/ad-anatomy/CLAUDE.md) |
...
| Video | Grok Imagine Video 1.5 in B-Roll; Gemini Omni Flash 1.1 in Playground; Continuous keeps Seedance 1.5 Pro |
...
- `projects` holds a NAME and nothing else — Playground's generations point at it (`projectId` on the image / video / music history row), never the other way round, so deleting one can't take a generation with it.
...
- `addToast(message, type, { label, run })` — a toast's ONE button is the next step (Connect Key, Undo, Open, Retry); the no-key / bad-key / no-credits sentences get theirs automatically, so reword them only through their constants in `friendlyError.ts`. The kie key guide has one host (the menu bar); open it with `openKeyGuide()`.
- Every overlay takes `useCloseOnEscape` and `useBackdropClose` (never `onClick={onClose}` on a backdrop); body-portaled ones also `useCloseOnAppSwitch`. `.modal-pop` / `.modal-fade` are how a centred one arrives, kept in step with `BankPicker`; a history rail slides (`.rail-slide-in` / `.rail-slide-out`, duration set by `RailOverlay`).
...
- `SectionCard` (don't card a lone control), `SectionPresetPill`, `VoiceCard` (B-Roll's; Playground keeps the profile inside its prompt box and shares only `VoicePresetPicker`), `StatusDot` (red means exactly "this is why Generate is grey"), `AutoGrowTextarea` and `BracketHighlightArea` (a field in a scrolling form grows, never scrolls), `PromptToolbar`, `ConstraintChip` (click-only; a duration chip renders through `DurationLabel`), `BatchCountStepper` (cap 4, Voiceovers 5; the total rides on the Generate button), `ClearAllButton` (inputs only), `DropOverlay`, `DayPill` (every list section label), `ModelPill`, `AppGlassTile`, `GridCanvas` (an empty stage, not a backdrop), `refInputParts` (a 64px `ImageTile` in a wrap row, never an aspect-square grid), `scriptBadge` (the house tinted chip, Title Case).
- History rails: `RailOverlay` is the panel — it slides in over the pane under the modal's own `bg-black/50` backdrop, dismisses on click-away, and has **no close of its own**, so a rail host is `relative` **and `md:overflow-hidden`** (the panel slides on a `translateX` from outside it, and an unclipped pane overflows the app shell into a transient horizontal scrollbar that shortens every column on screen) and acting in it — picking a row, or pressing `RailNewButton` — always hands the pane back. `RailNewButton` leads its band and is **one click in every app** — no arm, no Confirm, `HistoryRailToggle` opens it and leads the pane's own `h-[57px]` header, and `useHistoryRailOpen` holds the state — **never persisted**, or a reload lands on a panel over your work. **A history panel beside an input is `memo`'d with stable props.**
