...
- **Refresh-safe drafts + honest progress UI.** Every app's working state (inputs, generated-but-unsaved outputs, slot configs, prompt bar) now survives a page refresh via a new [`usePersistedState`](src/hooks/usePersistedState.ts) hook backed by localStorage, keyed per active project so drafts switch with the project chip. Ad Analyzer additionally saves the uploaded ad to IndexedDB through `assetStore.saveAsset` so the video survives — previously it lived only as a blob URL. In-flight kie jobs (taskIds, polling) are deliberately *not* persisted; resuming polling across reload is a separate, larger task. Paired with this: the old 95%-stuck progress bar in [`GenerationProgress`](src/components/GenerationProgress.tsx) is replaced with an indeterminate shimmer band (`@keyframes shimmer-sweep` in `index.css`) + rotating narration + a static "This can take a couple of minutes — keep this tab open" line. Elapsed-second counters on the Video Studio and Playground in-flight tiles are removed — both anxiety-inducing patterns trained users to refresh, which is why persistence matters.
- **Playground app (Image / Video / Music).** New free-form surface at the bottom of TOOLS. Single ElevenLabs-style prompt bar with three mode tabs, per-mode `ModelPicker` (keys `playground:image` / `playground:video` / `playground:music`), drag-drop ref upload, and `@`-mention autocomplete over Products / Characters / B-Rolls. Six curated UGC-ad preset cards in Video mode (Street interview, Podcast clip, Car rant, Mirror selfie review, Product shot b-roll, Live reaction unboxing); 5 of 6 lock the iPhone-UGC trailer, Podcast uses a cinematic interview trailer. Music tab adds Suno V5 / V5.5 via kie.ai's `/api/v1/generate` endpoint pair — see kie client section 4. Playground images piggyback on the `brolls` bank with no product/model/script linkage; videos go to `videoHistory`; tracks go to the new `musicHistory` bank.
