...
- **kie.ai API key — Supabase exposure fix.** The Bearer key used to live in `profiles.kie_api_key` (plain text). The `profiles_admin_read` RLS policy grants admins `select` on every column, so the admin (and anyone with Supabase Studio / service-role access) could read every member's key. Fixed by making the key browser-local only: [`saveProfile`](src/lib/cloudSync.ts) now pushes just `per_app_model`; [`hydrateFromCloud`](src/lib/cloudSync.ts) reads just `per_app_model` and preserves the in-memory `kieApiKey` so [`loadFromStorage`](src/stores/settingsStore.ts) remains the only source. [`setKieApiKey`](src/stores/settingsStore.ts) no longer calls `pushProfile()`. [`ProfileRow`](src/stores/authStore.ts) + `fetchProfile`'s select drop `kie_api_key`. Migration [`0008_drop_kie_api_key.sql`](supabase/migrations/0008_drop_kie_api_key.sql) nulls the column then drops it (guarded with an `information_schema` check, idempotent). [`PrivacyPolicy.tsx`](src/legal/PrivacyPolicy.tsx) §1 + §7 rewritten to disclose the local-only model; [`POLICY_VERSION`](src/legal/version.ts) bumped to `2026-05-27` so every signed-in user re-accepts via `LegalAcceptModal`. [`SettingsModal.tsx`](src/components/SettingsModal.tsx) adds a one-line "Stored only in this browser…" helper under the key input. Trade-off the user has accepted: re-paste required on each new browser/device.
- **Legal layer + acceptance tracking.** Added a Legal protection surface so the operator has Terms / Privacy / AUP / DMCA in-app and provable acceptance per user. Four docs live under [`src/legal/`](src/legal/) as plain TSX (no markdown lib) — [`TermsOfService.tsx`](src/legal/TermsOfService.tsx), [`PrivacyPolicy.tsx`](src/legal/PrivacyPolicy.tsx), [`AcceptableUsePolicy.tsx`](src/legal/AcceptableUsePolicy.tsx), [`DMCAPolicy.tsx`](src/legal/DMCAPolicy.tsx) — sharing [`LegalLayout.tsx`](src/legal/LegalLayout.tsx) (header + TOC sidebar + "Last updated" stamp). Copy is templated with `[REPLACE: …]` placeholders for entity name, jurisdiction, contact email, DMCA agent + `<Marker>Lawyer review</Marker>` callouts; it is NOT live legal copy. [`src/App.tsx`](src/App.tsx) mounts `/legal/terms /privacy /aup /dmca` as `Routes` outside `AuthGate` so signed-out visitors can read; everything else lives under the catch-all `*` route that wraps the workspace in `AuthGate`. [`POLICY_VERSION`](src/legal/version.ts) is the single source of truth — bumping it forces re-accept. [`AuthScreen.tsx`](src/components/auth/AuthScreen.tsx) adds a required agreement checkbox on the signup tab (button stays disabled until ticked) + a Terms · Privacy · AUP footer; after a successful signup with an immediate session it calls `acceptPolicies(POLICY_VERSION)`. [`LegalAcceptModal.tsx`](src/components/LegalAcceptModal.tsx) is a blocking modal shown to logged-in users whose `policy_version_accepted` lags the constant (covers email-confirm signups + future bumps). [`CookieBanner.tsx`](src/components/CookieBanner.tsx) is a one-time dismissible bottom-center banner explaining localStorage + IndexedDB, dismissal stored under `ugc-lab:cookie-consent`. [`authStore.ts`](src/stores/authStore.ts) ProfileRow + SELECT extended with `tos_accepted_at` / `privacy_accepted_at` / `aup_accepted_at` / `policy_version_accepted`; new `acceptPolicies(version)` action writes all four. Migration [`0007_legal_acceptance.sql`](supabase/migrations/0007_legal_acceptance.sql) adds the columns (idempotent). [`AiContentDisclosure.tsx`](src/components/AiContentDisclosure.tsx) renders a one-line "AI-generated content — verify before publishing or distributing" under the Characters, B-Roll, and Playground history surfaces. Sidebar bottom adds a muted "Terms · Privacy · AUP" link cluster (expanded sidebar only).
- **B-Roll polish + history isolation + the asset:// URI fix.** Several adjacent fixes hit the B-Roll card detail modal. (1) [`isAssetRef`](src/utils/assetStore.ts) and the read paths (`getBlob` / `getUrl` / `deleteAsset`) now accept both bare ids (`asset-xxx`) and `asset://`-prefixed refs — the image path stores bare, the video path stores prefixed, and only the bare form was being recognised, so every B-Roll video rendered as a black `<video>` element that had been handed the raw `asset://` URI as its src. The new `assetIdFromRef` helper strips the scheme; both forms round-trip. THIS was the root cause of the user's "black video" reports. (2) `InFlightTile` in [`CardDetailModal.tsx`](src/apps/broll-studio/components/CardDetailModal.tsx) now sizes to the entry's `aspectRatio` and uses the orange palette to match the page accent. (3) `VideoTile` keeps the previous hover-autoplay aesthetic plus a clickable play/pause overlay top-left — `<video>` has no native `controls`, the user clicks the badge to toggle and `e.stopPropagation` keeps the tile-click cover-select working. (4) `finishVideoTask` in [`generateVideo.ts`](src/apps/broll-studio/services/generateVideo.ts) now runs `probeVideoBlob` after the size + content-type guards — it hands the downloaded blob to a hidden `<video>`, waits for `loadedmetadata`, and rejects anything with non-finite or near-zero duration (catches kie content-filter placeholders, CORS-opaque stubs, HTML error pages tagged as `video/mp4`). `saveAsset` in [`assetStore.ts`](src/utils/assetStore.ts) adds a 0-byte guard. (5) `runVideoTask` in [`VariationCard.tsx`](src/apps/broll-studio/components/VariationCard.tsx) falls back to **text-to-video** (and drops the refs with a toast) when the selected video model lacks `reference-to-video`. We deliberately do NOT promote refs to a first-frame seed — that produced visibly distorted clips where the character was locked into the static reference pose. Use Veo 3.1 Fast or Seedance 2.0 to keep refs as refs. `ReferenceSlotCard` dims with a tooltip when the current video model doesn't support refs. The in-flight indicator on the card face is now driven by `cardState.inFlightVideos.length > 0` (was the stale `videoStatus === 'generating'`), and the card face shows an orange `GenerationProgress` block when a video is rendering and there's no cover yet. (6) The [`generateBroll.ts`](src/apps/broll-studio/services/generateBroll.ts) parser strips `<LABEL>` / `<REFS>` / `<PROMPT>` / `<VAR_n>` wrappers from `variation.prompt` even on partial LLM output; `migrateVariation` + the `cardStates` sanitize hook in [`BrollStudio.tsx`](src/apps/broll-studio/BrollStudio.tsx) wash the same tags out of any persisted prompts on hydrate. (7) Empty variation cards now render a 4-line snippet of the prompt + an uppercase "Click to set up" pill. (8) Two-click delete confirm added to B-Roll History rows, Playground tile delete buttons (shared `DeleteConfirmButton`), `AudioTile`, the modal's `TileDeleteButton`, and Character Studio's gallery delete. (9) `PlaygroundHistoryGrid` filters `videoHistory` to skip entries with `sourceApp === 'broll-studio'` — shared bank stays intact for refresh-resume, but the Playground grid only renders Playground gens. (10) OR dividers read context-specific labels in the original uppercase-muted style: "or paste script manually" (B-Roll, Voiceovers) / "or paste transcript manually" (Scripts).
## When making changes
