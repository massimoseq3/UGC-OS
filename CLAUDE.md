...
- **Bank writes: local-first + hard cloud-sync timeout.** Two rounds. **Round 1 (history banks):** the auto-history `add*` / `update*` actions in [`bankStore.ts`](src/stores/bankStore.ts) (`voiceHistory` / `videoHistory` / `imageHistory` / `musicHistory` / `characterHistory` / `adAnatomyHistory`) used to `await pushRow(...)` before the local `set()`. When Supabase upsert hung, the `set()` never ran, the row was lost on reload via [`hydrateFromCloud`](src/lib/cloudSync.ts) (which replaces local with cloud state), and the user just saw the success toast — fired by `finishGen` after the un-awaited add — with no gallery tile. Reported as "Characters generates, kie.ai log confirms, but the tile never appears even after reload." Fixed by flipping the order: `set()` first (synchronous localStorage + gallery update), then `pushRow` in the background. **Round 2 (CRUD banks):** same anti-pattern lived in `addProduct` / `addModel` / `addScript` / `addVoice` / `addBRoll` + their `update*` peers, surfacing as "click Save to Bank tick, spinner is stuck forever" — `commitSave` in [`GalleryPanel.tsx`](src/apps/character-studio/components/GalleryPanel.tsx) awaits `addModel` which awaits the hung `pushRow`. Flipped these too. **Plus a hard timeout:** `pushRow` and `dropRow` now wrap `saveRow` / `deleteRow` in a 15s `withTimeout` so any stalled fetch (mid re-auth, extension blocking the request, dropped connection that never RSTs) surfaces as an error toast within a bounded wait instead of leaving callers hanging indefinitely. On a timeout the local state is still correct because `set()` already ran. `reportError`'s re-throw behaviour is preserved so CRUD callers (e.g. `commitSave`) see the failure and clear their spinners via `finally`. `delete*` / `clear*` actions deliberately left as-is — those are explicit user intent where waiting on the network is the right UX, and the timeout applies to their `dropRow` too so they can't hang forever either. Matches the documented contract "Stores stay localStorage-backed (source of truth in the browser)" and is consistent with how [`saveAsset`](src/utils/assetStore.ts) treats the R2 mirror.
## When making changes
