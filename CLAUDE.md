...
- **History banks: local-first write order so cloud hangs can't swallow generations.** Reported as "Characters generates, success toast fires, kie.ai log confirms — but the tile never appears in the gallery, even after reload" — cloud-mode only. The `add*` / `update*` actions in [`bankStore.ts`](src/stores/bankStore.ts) for `voiceHistory` / `videoHistory` / `imageHistory` / `musicHistory` / `characterHistory` / `adAnatomyHistory` previously did `await pushRow(...)` BEFORE the local `set()`. If the Supabase upsert hangs (mid re-auth, network stall, extension blocking the request), the inner `set()` never runs, so the row never lands in local state or localStorage — and on reload [`hydrateFromCloud`](src/lib/cloudSync.ts) replaces local with what's on the server (nothing). The toast still fires because [`finishGen`](src/apps/character-studio/CharacterStudio.tsx) calls `addCharacterHistory(...)` un-awaited. Fix: flip the order — `set()` first (synchronous localStorage write + gallery re-render), then fire `pushRow` in the background; failures still surface as a `reportErrorSoft` toast but no longer block local state. Matches the documented contract "Stores stay localStorage-backed (source of truth in the browser)" and is consistent with how [`saveAsset`](src/utils/assetStore.ts) treats the R2 mirror. CRUD banks (products / models / scripts / voices / brolls) deliberately left as-is — they use the re-throwing `reportError` so callers can react, and flipping them would change `BankActionResult` semantics. `delete*` / `clear*` also left as-is — those run on explicit user action where waiting on the network is the right UX.
## When making changes
