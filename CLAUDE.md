...
| Tools | Ad Analyzer | `ad-anatomy/` | Ad image or video frame → scorecard + creative breakdown (hook / angle / structure + a product-agnostic Script Style Prompt, saveable to the Script Bank as `kind: 'style'`) + transcript + visual playbook |
...
| Image (text→image) | Nano Banana 2 (`nano-banana-2`) | App-wide default (leads the registry); Influencers defaults to GPT Image 2 (`gpt-image-2-text-to-image`). Picker also: GPT Image 2, Seedream 5 Lite |
