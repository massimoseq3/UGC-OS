...
| Create | Influencers | `character-studio/` | Form → portrait image, or (Portrait / Character Sheet toggle) a 16:9 multi-panel reference sheet (turnaround + expressions + full body; scene/pose/camera fields ignored; aspect locked 16:9, resolution bumps to 2K). Sheets attach to an existing bank Model as `sheetImage` instead of creating a new entry. Drop a reference photo anywhere to auto-fill every field via vision-based DNA extraction. Display name rebranded from "Characters" — folder, ids, types, and LLM prompts keep the character naming. |
...
| Video | Seedance 2.0 (`bytedance/seedance-2`); B-Roll defaults to Veo 3.1 Fast (`veo3_fast`) | Ref-capable. Picker also: Seedance 2.0 Fast, Veo 3.1 Fast/Lite/Quality, Gemini Omni (`gemini-omni-video`), Kling 3.0, Wan 2.7, Sora 2, Sora 2 Pro |
