...
- History rails: `RailOverlay` is the panel — it slides in over the pane under the modal's own `bg-black/50` backdrop, dismisses on click-away, and has **no close of its own**, so a rail host is `relative` and picking a row always hands the pane back. `RailNewButton` leads its band (arms only when it clears inputs; armed state is monochrome), `HistoryRailToggle` opens it and leads the pane's own `h-[57px]` header, and `useHistoryRailOpen` holds the state — **never persisted**, or a reload lands on a panel over your work. **A history panel beside an input is `memo`'d with stable props.**
