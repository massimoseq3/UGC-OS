...
- **Session freshness.** Every cloud write awaits `ensureFreshSession()` in `src/lib/supabase.ts`. It races Supabase `getSession()` against a 3s timeout and falls back to a module-cached access token (kept current by an `onAuthStateChange` listener) when the SDK's auth lock stalls — which it can after a backgrounded tab returns. This stops a hung `getSession()` from pinning writes for 15–60s (the old "generation fails until I refresh" symptom). The `visibilitychange` handler calls `ensureFreshSession()` to recover the token proactively on tab focus.
