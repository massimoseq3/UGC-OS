...
Dock order: **Dashboard** (own 'system' group) | **Bank** | **Ad Analyzer + the Create apps** (one group, analyzer leads — analyze first, then produce) | theme + Settings utility tiles. Fresh visits land on the Dashboard (`DEFAULT_SLUG`). Display names are terse nouns, not action verbs. Folder names and the `id` strings in `src/utils/constants.ts` are **stable** — they key per-app model selections in localStorage. Do not rename them.
...
| Create | Voiceovers | `voice-studio/` | Script → audio. Model picker: **ElevenLabs Multilingual v2** (default + Recommended — no prompt engineering, full speed/stability/similarity/style sliders) or **Eleven v3** (Text to Dialogue — more expressive, discrete stability toggle + an **Enhance** action that LLM-injects `[audio tags]`) |
...
| TTS | ElevenLabs Multilingual v2 (`elevenlabs/text-to-speech-multilingual-v2`); v3 selectable | Voiceovers Model picker (`ModelPickerView`). V2 default (flat `{ text, voice, stability 0–1, similarity_boost, style, speed }`, Recommended — no prompt engineering). V3 (`elevenlabs/text-to-dialogue-v3`) sends a `dialogue: [{ text, voice }]` array + discrete `stability` (0 Creative / 0.5 Natural / 1 Robust); audio tags like `[excited]` in the text drive delivery — the editor's **Enhance** button (V3 only) runs an LLM pass (`enhanceForV3`) that weaves tags into the script. ~64-voice catalog shared across both |
...
- **`VoicePreset` / `VoiceHistoryItem`** carry `modelId` + the full param set (`voiceId`, `stability`, `similarityBoost`, `style`, `speed`). `migrateVoiceShape` backfills `modelId` to V2 (legacy rows predate the picker), backfills missing params (`0.5 / 0.75 / 0 / 1`), and snaps `stability` onto the discrete grid **only for V3 rows** (V2 keeps continuous). `VoiceSettings` is likewise repaired on hydration by `normalizeSettings` (usePersistedState `sanitize`) so an old saved draft can't send an undefined `modelId` to kie.
