...
| Create | B-Roll | `broll-studio/` | Script → scenes → 4 variations/scene. VAR_1 is always Dialogue (lip-sync anchor); the other 3 are LLM-picked **distinct** roles per line from an 8-role menu (Action / Emotional / Product / POV / Environment / Transition / Proof) — parser falls back to the legacy fixed buckets when `<TAG>` is absent. Every image/video gen gets a deterministic iPhone-realism suffix appended at request time (`services/realism.ts`). Each card has Generate Image **and** Generate Video; video gens are fire-and-forget with refresh-safe resume. Video resolution follows the picked model's `videoConstraints.default` — fresh cards seed from it and the card modal snaps to it on a model flip (Gemini Omni → 1080p, which costs the same as 720p). |
